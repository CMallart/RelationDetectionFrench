...
def get_lookup_tables_xu(df):
    
    #for relation number
    relations = list(properties_names.keys())
...
def data_reshape_ours(df, lookup_table_relations, lookup_table_ent_types, lookup_table_deptags, lookup_table_postags, len_max_seq=30):
...
def data_reshape_xu(df, lookup_table_relations, lookup_table_deptags, lookup_table_postags, lookup_table_vocab, maxlen=30):
    #["id1", "id2", 'entity1', 'entity2',"word_path1", "word_path2", "dep_path1", "dep_path2", "pos_path1", "pos_path2","original_sentence", relation"]
    
    global num_classes
    num_classes = len(list(properties_names.keys()))
    
    #Create array X and y
    #y is of shape the number of samples
    y = []
    #X is of shape (number of samples, number of timesteps, number of features)
    features_words1 = []
    features_words2 = []
    features_pos1 = []
    features_pos2 = []
    ...
    for row in df.itertuples(): #ie for each sample
        y.append(lookup_table_relations[row.relation])
        ...
        #features = [seq_word1, seq_word2, seq_dep1, seq_dep2, seq_pos1, seq_pos2]
...
def sets_test_train(feats_file_ours, feats_file_xu):

    df = pd.read_csv("/home/cyrielle/Codes/clean_code/Features/data/features_ratio1_definitive_model12_.csv")
    df.set_index(["entity1","entity2", "original_sentence"], inplace=True)
    df.fillna(value="", inplace=True)

    dfn = pd.read_csv("/home/cyrielle/Codes/clean_code/Features/data/features_ratio1_definitve_XuBiLSTM.csv")
    dfn.fillna(value="", inplace=True)
    dfn.set_index(["entity1","entity2", "original_sentence"], inplace=True)

    with open('indexes_test.json') as json_file:
        ind_test = json.load(json_file)
        df_test = df[df.id_entities_pair.isin(ind_test)]
        dfn_test = dfn[dfn.id_entities_pair.isin(ind_test)]
        
    with open('indexes_train.json') as json_file:
        ind_train = json.load(json_file)
        df_train = df[df.id_entities_pair.isin(ind_train)]
        dfn_train = dfn[(dfn.id_entities_pair.isin(ind_train)) & (dfn.relation != "")]
 
    return(df_train, df_test, dfn_train, dfn_test)
...
def build_model_xu(X_train):
    ...
    softmax = Dense(num_classes, activation='softmax')(main_hidden)

    #define model
    model = Model(inputs=[input_word1, input_word2, input_dep1, input_dep2, input_pos1,input_pos2], outputs=softmax)#sigmoid)
    #compile model
    adam = optimizers.Adam(lr=0.001) #, kernel_regularizer=regularizers.l1(0.0001))
...
def test_model(trained_model, X_test, X_test_seq, y_test, result_folder):
    #predict the existence of relatins with model1
    

    #then classify with model2
...
@click.option(
    '--cutting_point',
    type=click.FLOAT,
    help = 'Minimum probability output by our model after which we consider a relation to exist. Defaults at 0.2 as used in our paper.',
    default = 0.2
)
def build_and_train_ours(feats_file_ours, feats_file_xu, result_folder, cutting_point):
    df_train, df_test, dfx_train, dfx_test= sets_test_train(feats_file_ours, feats_file_xu)

    #train set ours
    lookup_table_relations, lookup_table_ent_types, lookup_table_deptags, lookup_table_postags = get_lookup_tables_ours(df_train)
    dataX, dataXseq, datay = data_reshape_ours(df_train, lookup_table_relations, lookup_table_ent_types, lookup_table_deptags, lookup_table_postags) 
    X_train, X_val, X_train_seq, X_val_seq, y_train, y_val = train_test_split(dataX, dataXseq, datay, test_size=0.25, random_state=7)
    #test set ours
    X_test, X_test_seq, y_test = data_reshape_ours(df_test, lookup_table_relations,lookup_table_ent_types, lookup_table_deptags, lookup_table_postags) 

    #train set xu
    lookup_table_relationsx, lookup_table_deptagsx, lookup_table_postagsx, lookup_table_vocabx = get_lookup_tables_xu(dfx_train)
    dataX, datay = data_reshape_xu(dfx_train, lookup_table_relationsx, lookup_table_deptagsx, lookup_table_postagsx, lookup_table_vocabx) 
    index_train, index_val = train_test_split(range(0, len(datay)), test_size=0.25, random_state=7)
    X_trainx = [[dataXj[i] for i in index_train] for dataXj in dataX]
    X_valx = [[dataXj[i] for i in index_val] for dataXj in dataX]
    y_trainx = np.array([datay[i] for i in index_train])
    y_valx = np.array([datay[i] for i in index_val])

    #build and train the two models
    model_1 = build_model_ours(X_train, X_train_seq)
    trained_model_1 = train_model_ours(model_1, X_train, X_val, X_train_seq, X_val_seq, y_train, y_val, result_folder)
    
    model_2 = build_model_xu(X_trainx)
    trained_model_2 = train_model_xu(model_2, X_trainx, X_valx, y_trainx, y_valx, result_folder)
  
    #apply first model to get the predictions of existence on test set
    y_pred_bin = trained_model_1.predict([X_test, X_test_seq])
    y_pred_bin = y_pred_bin.flatten()
    y_pred_bin_cut = y_pred_bin > cutting_point

    indexes_in_df_test = df_test.id_entities_pair.tolist()
    indexes_true = [indexes_in_df_test[i] for i in list(np.nonzero(y_pred_bin_cut)[0])] # * - we can do that lineraly simply bc the order of rows is kept
    indexes_false = [indexes_in_df_test[i] for i in list(np.where(y_pred_bin_cut == 0)[0])]

    #separate second dataset according to predictions of relation existence
    df2_thrown = dfx_test[dfx_test.id_entities_pair.isin(indexes_false)]
    reel_none = [properties_names[r] for r in df2_thrown['relation'].to_list()]
    predictions_none = ["None"] * len(reel_none)

    df2_real = dfx_test[dfx_test.id_entities_pair.isin(indexes_true)]
    X_testx, y_testx = data_reshape_xu(df2_real, lookup_table_relationsx, lookup_table_deptagsx, lookup_table_postagsx, lookup_table_vocabx)
    y_true_classes = np.asarray(y_testx).argmax(axis=-1)
    y_pred = trained_model_2.predict(X_testx)
    ...
    total_predictions = predictions + predictions_none
    total_reel = reel + reel_none

    save_classif_reports(total_reel, total_predictions,report_file= None, print=True)
...
if __name__ == "__main__":
    ...
    result_folder = '/home/cyrielle/Codes/clean_code/Results/Relation_detection/Xu_LSTM/def_features/'
    feats_file_ours = "/home/cyrielle/Codes/clean_code/Features/data/definitive/features_ratio1_definitive_model12_.csv"
    feats_file_xu = "home/cyrielle/Codes/clean_code/Features/data/features_ratio1_definitve_XuBiLSTM.csv"

    build_and_train_ours(feats_file_ours, feats_file_xu, result_folder)
